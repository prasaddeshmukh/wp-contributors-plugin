	...
	?>
				...
				<div id="contributors-list">
					<p>
						...
						<?php
							//specific checkboxes will be 'on' if term_id matches the term_taxonomy_id present in wp_term_relationships table
...
/**
 *'save_post' hook
 *update wp_term_relationships table by inserting term_taxonomy_ids along with the post(object_id) when the post is published or 
 *updated.
*/

	function update_post_contributors($post_id, $post) {
   	...
   	if( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return; 
    	...
    	// if our nonce isn't there, or we can't verify it, bail   
  		if( isset( $_POST['contributors-nonce'] ) && isset( $_POST['contributors'] ) ) {
 ...
 */
	...
	function add_contributors_box( $content ) {
		...
		if(is_singular('post')) {
			...
			//displays original author(the one who created the post) along with his/her gravatar
			...
			//loops 'n' number of times, where 'n' is a number of contributors (terms belonging to 'contributors' taxonomy) on the post(object_id).  
